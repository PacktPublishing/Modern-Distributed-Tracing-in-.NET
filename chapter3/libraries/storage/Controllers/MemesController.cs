...
{
    private static readonly DistributedCacheEntryOptions ExpirationOptions = new DistributedCacheEntryOptions()
    {
    ...
    {
        var cachedStream = await GetFromRedisAsync(name, cancellationToken);
        if (cachedStream == null)
        {
            ...
            await _cache.SetAsync(name, copy.ToArray(), ExpirationOptions, cancellationToken);
            copy.Position = 0;
            cachedStream = copy;
        }
    ...
    private async Task<Stream?> GetFromRedisAsync(string name, CancellationToken cancellationToken)
    {
        try
        {
            var redisData = await _cache.GetAsync(name, cancellationToken);
            if (redisData != null)
            {
